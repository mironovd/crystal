...
def Step(I,js,C,n,wc,mutate_node_func,St,Gs,badroot,t,accumulator,sinks,stats,bi_first_neg_allow):
    St.append([])
    nodes=St[n]
    all_nodes = list(chain.from_iterable(St))
    all_nodes_set = Set([tuple([x.degree(tt) for tt in t]) for x in all_nodes])
    res=[]
    new_nodes = [[node,mutate_node_func(I,js,C,wc,node,badroot,t,accumulator,sinks,stats,bi_first_neg_allow)] for node in nodes]
    ...
    if len(res)==0 and St[n]!=[1]:
        for old_node in St[n]+sinks:
                for l in I:
                    for k in crystal_op.RR(wc,old_node,l,t):# range(1,len(wc)+1):
                        if (old_node*crystal_op.Ainv(C,wc,l,k,t))==1:
                            res=[1]
