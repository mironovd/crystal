...
parser=argparse.ArgumentParser(description='Crystal calculations for C_n- comparison of combinatorial method and determinant method', prog='sage crystal_cn_comp.sage')
...
def f(w,r,k,x,t):
     ...
     if i<r:
         ...
         res=tuple([0]+[x[j] for j in range(1,(r-1)+1)]+
                   [(1/t[k])*x[r], t[k]*x[r+1]+x[r] ] +
                   [x[j] for j in range(r+2,(2*r)+1)])
     return res
...
def vmat(w,r,t):
    return Matrix([v(w,r,j,t)[1:(2*r+1)] for j in range(1,2*r+1)])
